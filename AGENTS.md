...
- **MAY**: GitHub App の「公開 OSS リポジトリ向け完全無料枠」で、API キーの登録が不要なもの (例: CodeRabbit の OSS 無料枠。公開リポジトリ向けにクレジットカード登録不要で利用でき、利用上限を超えても課金は発生せず、レート制限または Fair Usage 制限が適用される。課金が発生する `Usage-based Add-on` は Pro / Pro+ プラン向けであり対象外)。
...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、新規ファイルだけでなく追加・変更したすべてのファイルに含まれていない。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **SHOULD** 取得してください。事前に Issue でこの承認を得た例外は、1.1 節の自動クローズ (MUST NOT) の対象外として扱います。承認を得ていない有料サービス導入 PR は、内容の良し悪しにかかわらず 1.1 節の規定により自動的にクローズされます。
...
本ポリシーは本リポジトリ [`genzouw/uconv`](https://github.com/genzouw/uconv) に適用されます。本リポジトリの `AGENTS.md` を他の公開リポジトリへ配布・同期する運用は現時点で存在しないため、他リポジトリへ適用する場合は、当該リポジトリに個別に `AGENTS.md` を配置してください。
